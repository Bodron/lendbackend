...
export class ReviewsController {
  ...
  create(
    ...
    return this.reviewsService.create(this.getUserId(authorization), productId, dto);
  }

  private getUserId(authorization?: string) {
    const [type, token] = authorization?.split(" ") ?? [];
    if (type !== "Bearer" || !token) throw new UnauthorizedException("Lipseste tokenul de autentificare.");
