...
PushDeviceSchema.index({ lastSeenAt: 1 }, { expireAfterSeconds: 60 * 24 * 60 * 60 });
...
export class PushService {
  ...
  constructor(@InjectModel(PushDevice.name) private readonly devices: Model<PushDevice>, config: ConfigService) {
    const json = config.get<string>("FIREBASE_SERVICE_ACCOUNT_JSON");
    const useAdc = config.get<string>("FIREBASE_USE_ADC") === "true";
    if (!json && !useAdc) {
      this.logger.warn("Push disabled: configure FIREBASE_SERVICE_ACCOUNT_JSON or FIREBASE_USE_ADC=true.");
      return;
    }
  ...
  async register(userId: string, token: string, platform: string) {
    await this.devices.updateOne({ token }, { $set: { userId, platform, lastSeenAt: new Date() } }, { upsert: true }).exec();
  }
  ...
  async sendMessage(recipientId: string, messageId: string, productId: string, productTitle: string) {
    if (!this.app) return;
    try {
      const devices = await this.devices.find({ userId: recipientId }).select("token").lean().exec();
      for (let offset = 0; offset < devices.length; offset += 500) {
        ...
        const result = await getMessaging(this.app).sendEachForMulticast({
          tokens,
          notification: { title: "Mesaj nou • Lend", body: "Ai primit un mesaj nou. Apasă pentru a deschide conversația." },
          data: { type: "chat_message", recipientId, messageId, productId, productTitle: productTitle.slice(0, 150) },
          apns: { headers: { "apns-push-type": "alert", "apns-priority": "10" }, payload: { aps: { sound: "default", threadId: productId } } },
      ...
      this.logger.error("Push delivery failed; message remains available in chat.");
    }
  }
}
